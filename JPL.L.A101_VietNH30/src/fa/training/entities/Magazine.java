...
public class Magazine extends Publication implements Comparable<Magazine> {
	...
	public void display() {
		...
		System.out.println("Magazine{ author:" + this.author +
				", volumn:" + this.volumn + ", edition:" +this.edition
				+ ", year of publication:"+ super.getPublicationYear()
				+ ", publisher:" + super.getPublisher()
				+ ", date of publication:"
				+ formatter.format(super.getPublicationDate()) + "}");

	}
	@Override
	public int compareTo(Magazine m) {
		if(this.getVolumn()>(m.getVolumn())) {
			return 1;
		}else if(this.getVolumn() == (m.getVolumn())) {
			return 0;
		}else {
			return -1;
		}		
